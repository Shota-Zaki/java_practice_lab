(function () {
  ...
  function tagHref(tag) { return (window.APP_ROOT || "") + "app/search.html?tag=" + encodeURIComponent(tag); }
  ...
  const EXAM_REVIEW_ROUTES = [
    ...
    { keys: ["module"], label: "module-info", article: "articles/java-gold-module-info-check/index.html", practice: "courses/gold17/practice/gold17-comprehensive-final.html" },
  ...
  function currentUnit() {
    ...
    if (reviewMode === "wrong") return { id: "review-wrong", title: scopeTitle ? `${scopeTitle}の間違えた問題だけ復習` : "間違えた問題だけ復習", description: `${scopeText} 不正解だった問題だけを1問ずつ解き直します。正解できた問題は復習対象から外れます。`, page: location.pathname.split('/').pop() };
    ...
    if (reviewMode === "due") return { id: "review-due", title: scopeTitle ? `${scopeTitle}の今日の復習` : "今日の復習", description: `${scopeText} 不正解・自信低め・一定期間経過した問題を優先して出します。`, page: location.pathname.split('/').pop() };
...
このまま採点しますか？`)) return;
  ...
  function renderQuestionView(scrollTop) {
    ...
    if (!questions.length) {
      ...
      const reviewLinks = isReview ? `<div class="practice-tools retry-empty-actions"><a class="btn primary" href="${pageHref('dashboard.html')}">ダッシュボードへ戻る</a><a class="btn" href="${pageHref('random.html')}">ランダム演習へ</a><a class="btn ghost" href="${pageHref('review-wrong.html')}${scope && reviewUnitFilter ? `?unit=${encodeURIComponent(reviewUnitFilter)}` : ""}">不正解復習を再確認</a></div>` : "";
      root.innerHTML = isReview
        ...
        : `<p class="notice">問題が見つかりません。データ読み込み、またはこのページのUNIT_IDを確認してください。</p>`;
      updateProgressUi();
      return;
    }
    const idx = currentIndex();
  ...
  function renderSearchPage() {
    ...
    const initialKeyword = urlParams.get("q") || urlParams.get("keyword") || "";
    if (initialTag) document.getElementById("searchTag").value = initialTag;
    if (initialUnit) document.getElementById("searchUnit").value = initialUnit;
    if (initialDifficulty) document.getElementById("searchDifficulty").value = initialDifficulty;
    if (initialKeyword) document.getElementById("searchKeyword").value = initialKeyword;
    const run = () => {
